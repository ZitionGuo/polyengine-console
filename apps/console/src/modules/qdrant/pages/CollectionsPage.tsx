...
const CollectionOptimizationsPanel = ({ collectionName }: { collectionName: string }) => {
  const optimizationsQuery = useQuery({
    queryKey: ["qdrant", "collections", collectionName, "optimizations"],
...
const CollectionSnapshots = ({ collectionName }: { collectionName: string }) => {
  ...
  const snapshotsQuery = useQuery({
    queryKey: ["qdrant", "collections", collectionName, "snapshots"],
    queryFn: () => api.listCollectionSnapshots(collectionName),
  });
  const createSnapshotMutation = useMutation({
    mutationFn: () => api.createCollectionSnapshot(collectionName),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["qdrant", "collections", collectionName, "snapshots"] });
  ...
  const restoreSnapshotMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["qdrant", "collections", "overview"] });
      queryClient.invalidateQueries({ queryKey: ["qdrant", "collections", collectionName] });
...
const CollectionPointsPreview = ({
  ...
  const invalidatePointData = () => {
    queryClient.invalidateQueries({ queryKey: ["qdrant", "collections", collectionName, "points"] });
    queryClient.invalidateQueries({ queryKey: ["qdrant", "collections", collectionName] });
  };
  const pointsQuery = useQuery({
    queryKey: ["qdrant", "collections", collectionName, "points", limit, offset, activeFilterJson],
    queryFn: () =>
...
export const CollectionsPage = () => {
  ...
  useEffect(() => {
    document.title = selectedCollection
      ? `${selectedCollection} · Collections · Qdrant · PolyEngine Console`
  ...
  const closeCollectionDetails = () => {
    if (window.location.pathname !== "/qdrant/collections") {
  ...
  const collectionsOverviewQuery = useQuery({
    queryKey: ["qdrant", "collections", "overview"],
  ...
  const detailsQuery = useQuery({
    queryKey: ["qdrant", "collections", selectedCollection],
  ...
  const createIndexMutation = useMutation({
    ...
    onSuccess: () => {
      if (selectedCollection) {
        queryClient.invalidateQueries({ queryKey: ["qdrant", "collections", selectedCollection] });
      }
      setIndexOpen(false);
  ...
  const retryIndexMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["qdrant", "collections", variables.failure.collectionName],
      });
