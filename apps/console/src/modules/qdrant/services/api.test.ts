...
describe("api service", () => {
  ...
  it("loads the detailed collection overview without changing the base list endpoint", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(1, "/api/qdrant/collections", expect.any(Object));
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "/api/qdrant/collections?include_details=true",
      expect.any(Object),
    );
  });
  ...
  it("posts alias create and update mutations to the backend alias API", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "/api/qdrant/aliases/docs%2Flive",
      expect.objectContaining({
        method: "PATCH",
        body: JSON.stringify({
          new_alias_name: "docs_current",
  ...
  it("calls payload index create and delete endpoints", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "/api/qdrant/collections/docs/indexes",
      expect.objectContaining({
        method: "PUT",
        body: JSON.stringify({
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "/api/qdrant/collections/docs/indexes/metadata.source",
  ...
  it("builds snapshot download URLs and loads optimization progress", async () => {
    ...
    expect(api.collectionSnapshotDownloadUrl("docs 2026", "nightly #1.snapshot")).toBe(
      "/api/qdrant/collections/docs%202026/snapshots/nightly%20%231.snapshot",
    );
    await api.getCollectionOptimizations("docs 2026", 5);

    expect(fetchMock).toHaveBeenCalledWith(
      "/api/qdrant/collections/docs%202026/optimizations?completed_limit=5",
      expect.any(Object),
    );
  });
  ...
  it("uploads snapshot FormData without overriding the multipart content type", async () => {
    ...
    expect(url).toBe(
      `/api/qdrant/collections/docs%20copy/snapshots/upload?wait=true&priority=snapshot&checksum=${"a".repeat(64)}`,
    );
  ...
  it("calls full storage snapshot endpoints with encoded download and delete URLs", async () => {
    ...
    expect(api.storageSnapshotDownloadUrl("full backup #1.snapshot")).toBe(
      "/api/qdrant/snapshots/full%20backup%20%231.snapshot",
    );
    await api.deleteStorageSnapshot("full backup #1.snapshot");

    expect(fetchMock).toHaveBeenNthCalledWith(1, "/api/qdrant/snapshots", expect.any(Object));
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "/api/qdrant/snapshots?wait=true",
      expect.objectContaining({ method: "POST" }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      3,
      "/api/qdrant/snapshots/full%20backup%20%231.snapshot?wait=true",
      expect.objectContaining({ method: "DELETE" }),
    );
  });
  ...
  it("calls the points scroll endpoint", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/api/qdrant/collections/docs/points/scroll",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({
          limit: 25,
  ...
  it("calls the points query endpoint", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/api/qdrant/collections/docs/points/query",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({
          query: [0.1, 0.2],
  ...
  it("calls the points retrieve endpoint", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/api/qdrant/collections/docs/points/retrieve",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({
          ids: [1, "abc"],
  ...
  it("calls point count and facet endpoints with encoded collection names", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "/api/qdrant/collections/docs%20copy/points/count",
      expect.objectContaining({
        method: "POST",
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "/api/qdrant/collections/docs%20copy/points/facet",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({
          key: "source.type",
  ...
  it("calls the points delete endpoint with wait as query parameter", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/api/qdrant/collections/docs/points/delete?wait=true&ordering=strong",
      expect.objectContaining({
        method: "POST",
  ...
  it("calls the points upsert endpoint with points in the request body", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/api/qdrant/collections/docs/points?wait=true&ordering=medium",
      expect.objectContaining({
        method: "PUT",
        body: JSON.stringify({
          points: [{ id: 1, vector: [0.1, 0.2], payload: { source: "manual" } }],
  ...
  it("calls point payload overwrite and clear endpoints", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "/api/qdrant/collections/docs/points/payload?wait=true&ordering=medium",
      expect.objectContaining({
        method: "PUT",
        body: JSON.stringify({
          points: ["point-1"],
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "/api/qdrant/collections/docs/points/payload/clear?wait=true",
      expect.objectContaining({
        method: "POST",
